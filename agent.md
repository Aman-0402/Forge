...
- Phase 5: security hardening done (session revocation, forced password change in the API, login lockout, one-time invite/reset links, forgot password, signed expiring file links, scheduled announcement delivery, prod settings + JSON logging + request-id middleware). Dependency audit done (2026-09-17, `pip-audit` + `npm audit`, 0 vulnerabilities). Public marketing site added (2026-09-17, see decisions log). Backend-controlled site settings, marketing stats and a contact-message admin inbox added (2026-09-17). Popup notifications (SweetAlert2 confirms/prompts, Toastr toasts) replace all `window.confirm`/`prompt`/`alert` app-wide (2026-09-17). Dashboard theme switcher (light/dark/auto) added (2026-09-17), dark mode matches the marketing site's gold-on-black palette. Admin analytics dashboard added (2026-09-17): `reports/overview/` + `/admin/analytics` (recharts: 5 donuts, 2 bar charts, 6 stat tiles). Still open: per-course/per-student reports + CSV export, role dashboards for faculty/student, email + certificates, performance checks, deployment + CI, remaining frontend pages, accessibility pass, user guides.
