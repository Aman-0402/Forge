...
- Phase 5: security hardening done (session revocation, forced password change in the API, login lockout, one-time invite/reset links, forgot password, signed expiring file links, scheduled announcement delivery, prod settings + JSON logging + request-id middleware). Dependency audit done (2026-09-17, `pip-audit` + `npm audit`, 0 vulnerabilities). Public marketing site added (2026-09-17, see decisions log). Backend-controlled site settings, marketing stats and a contact-message admin inbox added (2026-09-17). Popup notifications (SweetAlert2 confirms/prompts, Toastr toasts) replace all `window.confirm`/`prompt`/`alert` app-wide (2026-09-17). Dashboard theme switcher (light/dark/auto) added (2026-09-17), dark mode matches the marketing site's gold-on-black palette. Admin analytics dashboard added (2026-09-17): `reports/overview/` + `reports/timeseries/` (?days=1-180) + `/admin/analytics`, tabbed (Overview/Users/Courses/Exams/Coding, 1-3 charts each rather than all 7 at once) with a line chart (daily new users/enrollments/exam attempts/code submissions, 7/30/90-day filter buttons) plus 5 donuts, 2 bar charts, 6 stat tiles. Still open: per-course/per-student reports + CSV export, role dashboards for faculty/student, email + certificates, performance checks, deployment + CI, remaining frontend pages, accessibility pass, user guides.
...
3. Phase 5 items found in Phase 3:
   - Schedule `sweep_overdue_attempts` and `deliver_scheduled_announcements` every minute (cron container in deployment).
...
| 2026-09-17 | Marketing site connected to the real backend where it made sense: `/contact` posts to a new public `contact/` endpoint (`ContactMessage` model, notifies every active admin); the header shows "Go to dashboard" instead of Log in/Join free once authenticated | User asked to connect frontend and backend "whatever possible"; Programs/Home/Techies content stays as dsaclone's own static copy, not tied to real course data — a bigger content-modelling change, not attempted here |
...
| 2026-09-17 | New `apps.core` singleton `SiteSettings` (`registration_open`, `maintenance_mode`, `maintenance_message`) and `MarketingStat` model, both public-GET/admin-write, plus a read-only admin view of `ContactMessage` | User asked for backend-controlled UI switches surfaced in the admin dashboard; picked the three concrete, bounded pieces (contact inbox, feature flags, the home page's 3 stat numbers) over an open-ended CMS |
| 2026-09-17 | Registration gate enforced server-side in `register_student` (403 `registration_closed`), not just hidden in the frontend | A UI-only gate is not a real control; the frontend also reflects it for UX |
| 2026-09-17 | Maintenance banner lives outside `.main-content` (which reserves fixed padding for the header) and drives the header's `top` via a `--mkt-banner-offset` CSS var measured off the banner's real rendered height | Placing it inside that container made it render behind the fixed header, invisible; found via Playwright + computed-style inspection, not just a screenshot glance |
| 2026-09-17 | Toastr requires a real global `jQuery`, not just an ES import; isolated into `utils/jquery-global.ts`, imported first, always before `toastr` | Toastr's UMD build checks `window.jQuery` at module-eval time; a bundler's module-scoped `import $ from "jquery"` alone doesn't set that global |
| 2026-09-17 | Every `window.confirm`/`prompt`/`alert` app-wide replaced with SweetAlert2 (`confirmDialog`/`promptDialog`/`alertDialog`) and Toastr (`toast.*`), in `utils/notify.ts` | User asked for these two libraries for "popup notifications"; native dialogs block the render thread and look inconsistent with the rest of the UI |
| 2026-09-17 | Fixed a real bug found during a UI review pass: the class-collision rename (2026-09-17 entry above) only updated `.css` selectors — the shell script's file-discovery grep required a literal `.` before the class name, which JSX `className="..."` strings never have, so `Programs.tsx` (`course-card`, `course-grid`) and `Home.tsx` (`eyebrow`) kept the un-prefixed names and rendered unstyled/wrong (white cards, invisible text) | Re-swept every marketing `.tsx` file directly for the 5 collision words as literal `className` content, not just CSS selectors, and fixed the 7 remaining spots; re-verified visually |
| 2026-09-17 | Dashboard theme switcher (`ThemeContext.tsx`, sidebar `ThemeToggle`): light/dark/auto, persisted in `localStorage`, applied via `documentElement.dataset.theme` and a blocking inline script in `index.html` (no flash on load) | User asked for a theme changer in the dashboard |
| 2026-09-17 | Dark mode re-points the app's own design tokens (`index.css` `:root`) to the marketing site's palette (near-black ground, gold `--blue` accent) instead of building a second, separate dark stylesheet | User asked for dark mode to visually match the public frontend; every colour in the authenticated app already ran through CSS variables, so repointing tokens covers the whole app with no per-component changes. Every previously-hardcoded hex color in `index.css` (status/wash/temper colours) was promoted to a token in the same pass, since an untokenized colour is invisible to a theme switch |
| 2026-09-17 | New `--on-accent` token for text painted on a solid `--blue`/`--straw` fill (buttons, badges, exam tick marks) | Swapping `--blue` from steel-blue to gold in dark mode broke the assumption baked into `color: #fff` on those fills; needs to flip to dark text on a bright gold fill |
| 2026-09-17 | `recharts` for the admin analytics dashboard; a fixed categorical colour palette for chart data (not theme tokens), but chart chrome (axes, grid, legend, tooltip) reads `var(--ink-3)` etc. so it still follows light/dark | Data-series colours are conventionally theme-invariant; only the surrounding UI needs to match the app |
| 2026-09-17 | `reports.overview()` counts active enrollments only for "top courses by enrollment", not all enrollments ever made | A completed/dropped enrollment shouldn't inflate "how many students are currently taking this course" |
